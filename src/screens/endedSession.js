            ...
            querySnapshot.forEach((docSnap) => {
              if (docSnap.exists()) {
                ...
                const unsubscribeDrinksCollection = onSnapshot(drinksCollectionRef, (drinksSnapshot) => {
                  ...
                  const BAC = calculateBAC(userData, drinks, sessionData.endedAt);
                  ...
                  userData.BACTrending = isUserBACTrending(userData, drinks, sessionEndTime);

                  const totalAlcoholGrams = calculateTotalAlcoholGrams(drinks, sessionEndTime);
                  const totalAlcoholGramsDataPoints = getTotalAlcoholGramsPoints(drinks, sessionStartTime, sessionEndTime);
                  ...
                  if (processedUsersCount === totalUsersCount) {
                    ...
                    setSessionUsers(userArray.sort((a, b) => b.BAC - a.BAC).map((user, index) => ({
                      ...user,
                      color: getRandomColor(index, totalUsersCount),
                    })));
                  }
